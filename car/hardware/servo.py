...
servo_max = 350 # Max pulse length out of 4096 / 600/492
...
class ServoPCA9685(object):
    ...
    def set_pulse(self, pulse):
        if pulse >= servo_min and pulse <= servo_max:
            self.pca9685.set_pwm(self.channel, 0, int(pulse))
            time.sleep(0.005)
...
def main():
    pca9685 = PCA9685(SMBus(1))
    servo = ServoPCA9685(pca9685, CHANNEL00)
    try:
        angle = servo_min
        while True:
            angle = int(input())
